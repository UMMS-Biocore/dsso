...
import { showInfoModal, createFormObj, fillFormByName, showFormError } from './jsfuncs';
...
const getAdminUserForm = type => {
  ...
  ret += getFormRow(getDropdown('loginType', ['password', 'google', 'ldap']), 'Login Type', {});
  ret += '</form>';
  return ret;
};
...
const bindEventHandlers = () => {
  ...
  $(document).on('click', `button.admin-add-user`, async function(e) {
    $('#crudModalError').empty();
    const form = getAdminUserForm('insert');
  ...
  $(document).on('click', `a.editUser`, async function(e) {
    ...
    $('#crudModalError').empty();
    const form = getAdminUserForm('edit');
